...
var keyObj=new THREE.Group(); keyObj.position.set(AX-2.1,0.10,AZ+0.4); keyObj.visible=false;
(function(){ var b=matGold;
  var s=new THREE.Mesh(new THREE.CylinderGeometry(0.018,0.018,0.30,10),b); s.rotation.z=Math.PI/2; keyObj.add(s);
  var o=new THREE.Mesh(new THREE.TorusGeometry(0.055,0.016,8,18),b); o.position.x=-0.18; o.rotation.y=Math.PI/2; keyObj.add(o);
  keyObj.add(new THREE.Mesh(new THREE.BoxGeometry(0.02,0.055,0.045),b).translateX(0.12).translateY(-0.03));
  keyObj.children.forEach(function(c){c.castShadow=true;}); });
scene.add(keyObj);
var keyGlow=new THREE.PointLight(0xffcf87,0,2.6,2); keyGlow.position.copy(keyObj.position).setY(0.3); scene.add(keyGlow);
var keyHit=new THREE.Mesh(new THREE.BoxGeometry(0.6,0.4,0.5), new THREE.MeshBasicMaterial({visible:false}));
...
act(rug,"la natte de roseaux",function(){
  ...
  keyObj.visible=true; keyHit.visible=true; keyGlow.intensity=2.6; ping(300,.25,.04);
  say("La natte se replie vers la droite. Une lueur de bronze sur le dallage, à gauche.");
});
...
(function(){
  ...
  for(var k=0;k<3;k++){
    ...
    var l=new THREE.PointLight(0xffb765,1.0,3.4,2); l.position.set(cx,1.8,cz); scene.add(l);
  }
...
// un doigt : trois phalanges articulées, refermées sur la crosse
function finger(par,x,y,z,curl,scale){
  var g1=new THREE.Group(); g1.position.set(x,y,z); par.add(g1);
  bone(g1,0.052*scale,0.0125*scale,0.0115*scale,skin);
  var g2=new THREE.Group(); g2.position.z=0.052*scale; g2.rotation.x=curl; g1.add(g2);
  bone(g2,0.036*scale,0.0115*scale,0.0105*scale,skin);
  var g3=new THREE.Group(); g3.position.z=0.036*scale; g3.rotation.x=curl*0.85; g2.add(g3);
  bone(g3,0.026*scale,0.0105*scale,0.0092*scale,skin);
  var n=new THREE.Mesh(new THREE.SphereGeometry(0.0085*scale,7,6), nail);
  n.position.set(0,0.006*scale,0.020*scale); n.scale.set(1,.5,1.4); g3.add(n);
  return g1;
}
...
(function(){
  ...
  var wr=new THREE.Mesh(new THREE.CylinderGeometry(0.055,0.040,0.17,10),
    new THREE.MeshStandardMaterial({color:0x6a5535,roughness:.98}));
  wr.rotation.x=Math.PI/2; wr.position.set(0,0.03,0.55); lHand.add(wr);
  var fl=new THREE.Mesh(new THREE.SphereGeometry(0.072,10,10),
    new THREE.MeshBasicMaterial({color:0xffd08a,transparent:true,opacity:.92}));
  fl.position.set(0,0.075,0.63); fl.scale.set(1,1.5,1); lHand.add(fl);
  lHand.userData.flame=fl;
})();
var torchLight=new THREE.PointLight(0xffb265, 7.5, 16, 2);
torchLight.position.set(-0.55,-0.15,0.1); camera.add(torchLight); scene.add(camera);
...
(function(){
  ...
  slide.position.set(0,0.042,0.005); gun.add(slide); gun.userData.slide=slide;
  // carcasse et crosse inclinée
  var frame=slab([[-0.100,-0.004],[0.070,-0.004],[0.070,0.014],[-0.100,0.014]], 0.050, 0.004, steel2);
  frame.position.set(0,0.005,0.005); gun.add(frame);
  var gp=slab([[-0.028,-0.150],[0.030,-0.120],[0.040,0.006],[-0.040,0.006]], 0.046, 0.006, grip);
  gp.position.set(0,-0.075,-0.052); gp.rotation.x=0.10; gun.add(gp);
  // pontet
  var tg=new THREE.Mesh(new THREE.TorusGeometry(0.030,0.0065,7,14,Math.PI*1.15),
    steel2); tg.position.set(0,-0.030,0.020); tg.rotation.set(0,Math.PI/2,-0.5); gun.add(tg);
  var tr=new THREE.Mesh(new THREE.BoxGeometry(0.010,0.030,0.008), steel2);
  tr.position.set(0,-0.018,0.012); gun.add(tr);
  // canon, guidon, hausse
  var br=new THREE.Mesh(new THREE.CylinderGeometry(0.0105,0.0105,0.030,12), steel);
  br.rotation.x=Math.PI/2; br.position.set(0,0.042,0.132); gun.add(br);
  part(gun,0.007,0.010,0.010, 0,0.077,0.115, steel2);
  part(gun,0.020,0.009,0.008, 0,0.077,-0.095, steel2);
  var fl2=new THREE.Mesh(new THREE.ConeGeometry(0.075,0.19,9),
    new THREE.MeshBasicMaterial({color:0xffeab4,transparent:true,opacity:0}));
  fl2.rotation.x=Math.PI/2; fl2.position.set(0,0.042,0.20); gun.add(fl2);
...
function viewmodel(dt,moving){
  ...
  if(gun.userData.slide) gun.userData.slide.position.z=0.005-slideBack*0.045;
...
function limb(par,len,r1,r2,mat){
  ...
  var c=new THREE.Mesh(new THREE.CylinderGeometry(r1,r2,len,9), mat);
  c.position.y=-len/2; c.castShadow=true; g.add(c);
  var j=new THREE.Mesh(new THREE.SphereGeometry(r2*1.05,8,6), mat);
...
function Mummy(x,z){
  ...
  var rags=[];
  for(var q=0;q<7;q++){
    var rg=new THREE.Mesh(new THREE.PlaneGeometry(0.055,0.30+Math.random()*0.26),
    ...
    rg.rotation.y=an; g.add(rg); rags.push(rg);
  }
  var hit=new THREE.Mesh(new THREE.BoxGeometry(0.78,1.95,0.78), new THREE.MeshBasicMaterial({visible:false}));
...
function scan(){
  ...
  var t=$("#tip");
  if(aim){ t.textContent="E · "+aim.userData.label; t.classList.add("on"); $("#dot").classList.add("hot"); }
  else{ t.classList.remove("on"); $("#dot").classList.remove("hot"); }
  var foe=false;
...
function loop(){
  ...
  if(keyObj.visible) keyObj.rotation.y+=dt*0.55;
  ...
  $("#where").textContent=RN[room()];
  if((fr&31)===0) $("#goal").textContent=goal();
  chrono(dt);
  renderer.clear();
